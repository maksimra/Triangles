...
double Vector::squareLength() const { return x * x + y * y + z * z; }
...
double Vector::mixedProduct(const Vector &v1, const Vector &v2,
                            const Vector &v3) {
...
Segment Segment::maxLengthSegment(const Segment &seg1, const Segment &seg2,
                                  const Segment &seg3) {
...
bool Segment::checkIntersectionOnSameLine(const Segment &seg1,
                                          const Segment &seg2) {
  if (seg1.isDegenerate() && seg2.isDegenerate())
    return seg1.p1 == seg2.p1;
...
bool Line::contain(const Point &p) const {
  return direction.collinear(
      Vector{p.x - point.x, p.y - point.y, p.z - point.z});
}
...
std::optional<Point> Line::getIntersection(const Line &other) const {
  ...
  if (!MathUtils::isZero(det * det, direction.squareLength() *
                                        other.direction.squareLength() *
                                        suppVec.squareLength()))
    return std::nullopt;

  auto solution = MathUtils::solve2x2Equation(
      direction.x, -other.direction.x, point.x - other.point.x, direction.y,
      -other.direction.y, point.y - other.point.y);
  if (!solution.has_value()) {
    solution = MathUtils::solve2x2Equation(
        direction.x, -other.direction.x, point.x - other.point.x, direction.z,
        -other.direction.z, point.z - other.point.z);
    if (!solution.has_value()) {
      solution = MathUtils::solve2x2Equation(
          direction.y, -other.direction.y, point.y - other.point.y, direction.z,
          -other.direction.z, point.z - other.point.z);
    }
  }
...
std::optional<Line> Plane::getIntersection(const Plane &other) const {
  ...
  std::optional<std::pair<double, double>> solutionZIs0 =
      MathUtils::solve2x2Equation(A, B, D, other.A, other.B, other.D);
  if (solutionZIs0) {
  ...
  std::optional<std::pair<double, double>> solutionYIs0 =
      MathUtils::solve2x2Equation(A, C, D, other.A, other.C, other.D);
  if (solutionYIs0) {
  ...
  std::optional<std::pair<double, double>> solutionXIs0 =
      MathUtils::solve2x2Equation(B, C, D, other.B, other.C, other.D);
  if (solutionXIs0) {
...
bool Triangle::isPoint() const { return p1 == p2 && p1 == p3; }
...
std::optional<Segment> Triangle::getIntersection(const Line &line) const {
  ...
  if (isDegenerate()) {
    ...
    if (intersecCandidate.has_value())
      return Segment{intersecCandidate.value(), intersecCandidate.value()};
    ...
    if (intersecCandidate.has_value()) {
      ...
      case 2:
        if (intersecCandidate.value() == intersecPoint1 ||
          ...
          break;
        assert(false &&
               "Wrong number line and triangle intersection points.\n");
      }
    }
  };
  ...
  default:
    assert(false &&
           "Unreachable number line and triangle intersection points.\n");
  }
}
...
bool Triangle::checkIntersection(const Point &p) const {
  ...
  case POINT:
    return p == p1;
  case SEGMENT: {
    Segment seg = Segment::maxLengthSegment(Segment{p1, p2}, Segment{p1, p3},
                                            Segment{p2, p3});
...
bool Triangle::checkIntersection(const Triangle &t, const Point &p) {
  auto inOneSide =
      [](const Point &p1, const Point &p2, const Point &p3,
         const Point &p4) // check if p1 and p2 on one side of the p3-p4
  {
    return Vector{p3, p4}.cross(Vector{p3, p2}) *
               Vector{p3, p4}.cross(Vector{p3, p1}) >=
           0;
  };
  ...
  if (!inOneSide(t.p1, p, t.p2, t.p3) || !inOneSide(t.p2, p, t.p1, t.p3) ||
...
bool Triangle::checkIntersection(const Triangle &t, const Segment &seg) {
  ...
  */
  ...
  auto solution =
      MathUtils::solve3x3Equation(MathUtils::Matrix3x3{E1, E2, d}, T);
  assert(solution.has_value());
  ...
  case TRIANGLE: {
    ...
    if (thisPlane.value() == otherPlane.value()) {
      if (checkIntersection(*this, other.p1) ||
          ...
          checkIntersection(*this, other.p3) || checkIntersection(other, p1) ||
          checkIntersection(other, p2) || checkIntersection(other, p3))
        return true;
      return false;
    }
    ...
    return Segment::checkIntersectionOnSameLine(thisSeg.value(),
                                                otherSeg.value());
  }
  default:
...
bool Triangle::trianglesIntersection(const Triangle &other) const {
  ...
  case POINT:
    return checkIntersection(other.p1);
  case SEGMENT:
    return checkIntersection(Segment::maxLengthSegment(
        Segment{other.p1, other.p2}, Segment{other.p1, other.p3},
...
Box::Box(const Point &p1, const Point &p2) {
...
bool Box::contain(const Point &p) const {
  return p.x >= bottomLeft.x && p.y >= bottomLeft.y && p.z >= bottomLeft.z &&
         p.x <= topRight.x && p.y <= topRight.y && p.z <= topRight.z;
}

bool Box::contain(const Triangle &t) const {
  return contain(t.p1) && contain(t.p2) && contain(t.p3);
}

double Box::size() const { return (topRight.x - bottomLeft.x) / 2; }

Point Box::getTopRight() const { return topRight; }

Point Box::getbottomLeft() const { return bottomLeft; }
} // namespace Geometry