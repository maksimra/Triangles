...
struct Vector {
  ...
  static double mixedProduct(const Vector &v1, const Vector &v2,
                             const Vector &v3);
  ...
  operator MathUtils::Vector3() const { return MathUtils::Vector3{x, y, z}; }
};
...
struct Segment {
  ...
  static Segment maxLengthSegment(const Segment &seg1, const Segment &seg2,
                                  const Segment &seg3);

  static bool checkIntersection(const Segment &seg1, const Segment &seg2);

  static bool checkIntersectionOnSameLine(const Segment &seg1,
                                          const Segment &seg2);
};
...
struct Line {
  ...
  Line(const Point &p1, const Point &p2) : direction(p2, p1), point(p1) {}

  Line(const Point &p, const Vector &v) : point(p), direction(v) {}

  Line(const Segment &seg) : point(seg.p1), direction(seg.p1, seg.p2) {}
...
struct Triangle {
  ...
  enum Type { POINT = 1, SEGMENT = 2, TRIANGLE = 3 };
...
public:
  ...
  bool contain(const Triangle &t) const;
