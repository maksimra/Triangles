...
TEST(UnitTest, LineIntersecSegment) {
  ...
  EXPECT_EQ(line1.getIntersection(segment1).value(), (Point{3, 2, 3}));
...
TEST(UnitTest, PlaneIntersecPlane) {
  ...
  auto result2 = plane3.getIntersection(plane4);
  ...
  auto result6 = plane11.getIntersection(plane12);
  ...
  auto result7 = plane13.getIntersection(plane14);
...
TEST(UnitTest, TriangleIntersecLine) {
  ...
  auto result1 = triangle.getIntersection(line1);
  EXPECT_TRUE(result1.has_value());
  ...
  // Прямая через вершину (1,2,0) с направлением (1,1,0) пересекает треугольник
  // только в вершине
  Line line2{Point{1, 2, 0}, Vector{1, 1, 0}};
  auto result2 = triangle.getIntersection(line2);
  EXPECT_TRUE(result2.has_value());
  EXPECT_EQ(result2.value(), (Segment{Point{1, 2, 0}, Point{1, 2, 0}}));

  // ТЕСТ 3: Прямая лежит в плоскости треугольника и проходит через две точки
  // (по отрезку) Прямая x=1, z=0 пересекает треугольник по отрезку от (1,0,0)
  // до (1,2,0)
  Line line3{Point{1, 0, 0}, Vector{0, 1, 0}};
  auto result3 = triangle.getIntersection(line3);
  ...
  auto result4 = triangle.getIntersection(line4);
  ...
  auto result5 = triangle.getIntersection(line5);
  ...
  auto result9 = degenerate_triangle.getIntersection(line9);
  EXPECT_TRUE(result9.has_value());
  EXPECT_EQ(result9.value(), (Segment{Point{0.5, 0, 0}, Point{0.5, 0, 0}}));

  // ТЕСТ 10: Прямая лежит в плоскости вырожденного треугольника и пересекает
  // его в точке
  Line line10{Point{0.5, 0, 0}, Vector{0, 1, 0}};
  auto result10 = degenerate_triangle.getIntersection(line10);
  ...
  auto result11 = degenerate_triangle.getIntersection(line11);
